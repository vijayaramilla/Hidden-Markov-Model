...
def individually_most_likely_states(X, pi, A, B):
    ...
    Z = np.empty((N,T), dtype=np.ndfloat64)
    for i in X.shape[0]:
        x = X[i,:]
        alpha = forward(x, pi, A, B)
        beta = backward(x, pi, A, B)
        p_x = np.sum(alpha) #vijay: recheck this normalization.
        p_z  = (1/p_x) * alpla.dot(beta) #vijay: recheck this dot product
        Z[i] = p_z
    return Z
